...
def Evaluating_Indicator(y_true,y_pred):
    ...
    F1 = metrics.f1_score(y_true,y_pred)#F1分数
...
def encode_onehot(labels):#将标签编码为one-hot形式
    classes = list(set(labels))#获取标签的种类
    classes.sort(key = list(labels).index,reverse=True)#按标签的索引进行排序
    #print(classes)
    classes_dict = {c: np.identity(len(classes))[i, :] for i, c in
                    enumerate(classes)}#将标签编码为one-hot形式
    labels_onehot = np.array(list(map(classes_dict.get, labels)),#将标签转换为one-hot形式
                             dtype=np.int32)
    return labels_onehot#返回one-hot编码的标签


def load_data(file_par, parti, path="C:\\project\\python\\pythonProject\\re\\keti\\dataset\\training data\\", dataset="HCGCN-all herb pairs"):#加载数据集
    ...
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float32)#第2列到倒数第2列，将特征转换为稀疏矩阵
    labels = encode_onehot(idx_features_labels[:, -1])#将所有行的最后一列标签转换为one-hot形式
...
'''
使用 indices、values 和 shape 创建 PyTorch 稀疏张量。
torch.sparse_coo_tensor 是 PyTorch 提供的函数，用于创建 COO 格式的稀疏张量。
'''