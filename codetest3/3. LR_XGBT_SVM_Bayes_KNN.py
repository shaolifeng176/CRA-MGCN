...
def write_roc_data_to_file(roc_data, model_name, fold):
    """写入ROC曲线数据到文件"""
    with open('roc_data_origin.csv', 'a') as f:
...
# 清空或创建ROC数据文件
with open('roc_data_origin.csv', 'w') as f:
    f.write("")  # 清空文件
...
# 计算平均ROC曲线数据并写入文件
with open('roc_data_origin_avg.csv', 'w') as f:
...
print("\nROC曲线数据已保存到 roc_data_origin.csv (每折数据) 和 roc_data_origin_avg.csv (平均数据)")
print("您可以在Origin中导入这些文件绘制ROC曲线:")
print("1. 使用 roc_data_origin.csv 绘制各折ROC曲线")
print("2. 使用 roc_data_origin_avg.csv 绘制平均ROC曲线")