...
# 文件路径
csv_file = r"C:\project\python\pythonProject\re\keti\3维柱状图\调气\调气英文.csv"
...
edge_width = 1.5  # 边的宽度
dpi = 800  # 图片分辨率
# 定义最外面两圈节点标签的倾斜角度，你可以自由调整这个值
label_rotation_angle = 20
...
for _, row in df.iterrows():
    ...
    if pd.notna(latin_name) and latin_name.strip():  # 检查Latin Name是否为空
        ...
        if pd.notna(row['Nature']):
            natures = [n.strip() for n in row['Nature'].split(',') if n.strip()]
        ...
        if pd.notna(row['Taste']):
            tastes = [t.strip() for t in row['Taste'].split(',') if t.strip()]
            for taste in tastes:
        ...
        if pd.notna(row['Meridian']):
            meridians = [m.strip() for m in row['Meridian'].split(',') if m.strip()]
...
# 修改划分比例，让更多节点到最外层
split_ratio = 0.45  # 可以调整这个比例，这里表示20%的节点到第三层，80%到最外层
split_index = int(len(outer_nodes) * split_ratio)
third_nodes = outer_nodes[:split_index]
outermost_nodes = outer_nodes[split_index:]
...
for node, (x, y) in pos.items():
    if node in third_nodes or node in outermost_nodes:
        plt.text(x, y, labels[node], fontsize=font_size,
                 fontfamily='Times New Roman',fontweight='bold',
                 ha='center', va='center', rotation=label_rotation_angle)
    else:
        plt.text(x, y, labels[node], fontsize=font_size,
                 fontfamily='Times New Roman', fontweight='bold',
                 ha='center', va='center')
...
# 保存图形
plt.savefig(r'C:\project\python\pythonProject\re\keti\3维柱状图\调气\herb_attribute_network.png', dpi=dpi, bbox_inches='tight')
