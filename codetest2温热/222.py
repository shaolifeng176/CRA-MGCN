...
model.load_state_dict(torch.load(r'C:\project\python\pythonProject\re\keti\codetest2温热\model_fold9_best.pth'))
model.eval()
...
merged_data = pd.DataFrame(columns=['Source_File', 'Original_Index', 'Predicted_Class', 'Probability', 'Chinese_Data'])
...
for i in range(1, 644):
    try:
        ...
        output_file = fr'C:\project\python\pythonProject\re\keti\codetest2温热\output\predictions{i}.csv'
        id_file = fr'C:\project\python\pythonProject\re\keti\dataset\HCGCN-ID for prediction sets\HCGCN-ID for prediction set-{i}.csv'
        ...
        # 筛选预测类别为1且概率超过0.8的数据
        filtered_df = df[(df['Predicted_Class'] == 1) & (df['Probability'] > 0.8)]

        # 添加来源文件列
        filtered_df['Source_File'] = f'predictions{i}.csv'
        ...
        # 创建一个映射字典，键为索引，值为对应的中文数据
        id_mapping = dict(zip(id_df.iloc[:, 0], id_df.iloc[:, 1]))

        # 根据 Original_Index 从映射字典中获取对应的中文数据
        filtered_df['Chinese_Data'] = filtered_df['Original_Index'].map(id_mapping)

        # 选择需要的列
        filtered_df = filtered_df[['Source_File', 'Original_Index', 'Predicted_Class', 'Probability', 'Chinese_Data']]
...
# 保存合并后的数据
merged_output_file = r'C:\project\python\pythonProject\re\keti\codetest2温热\output\merged_predictions9.csv'
