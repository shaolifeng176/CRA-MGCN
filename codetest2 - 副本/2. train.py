...
parser.add_argument('--epochs', type=int, default=50000, help='训练轮次')
...
def test(model, features, adj, labels, idx_test):
    ...
    # 修改这一行
    test_pred_proba = torch.softmax(output[idx_test], dim=1)[:, 1].detach().cpu().numpy()

    fpr, tpr, _ = metrics.roc_curve(test_true, test_pred_proba)
...
if __name__ == "__main__":
    ...
    for fold in range(5):
        ...
        adj, features, labels, idx_train, idx_test = load_data(
            path='C:\\project\\python\\pythonProject\\re\\keti\\dataset\\training data\\',
            dataset='HCGCN-all herb pairs',
            fold=fold
        )
        ...
        for epoch in range(args.epochs):
            ...
            # 每隔100轮验证一次
            if epoch % 100 == 0:
                test_metrics, fpr, tpr = test(model, features, adj, labels, idx_test)
    ...
    with open('roc_curve_data.csv', 'w') as f:
        ...
        for fpr, tpr in zip(mean_fpr, mean_tpr):
            f.write(f'GCNWithAttention,{fpr},{tpr}\n')