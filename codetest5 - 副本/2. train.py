...
def train(epoch):
    ...
    # 添加注意力可视化
    if epoch % 50 == 0 and args.cuda:
        with torch.no_grad():
            att_weights = model.get_attention_weights()
            node_att_mean = att_weights['node_layer1'].mean().item()
            print(f"Epoch {epoch}: Node attention mean={node_att_mean:.4f}")
